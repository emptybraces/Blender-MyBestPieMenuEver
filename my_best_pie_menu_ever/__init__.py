# ---------------------------
# 1.6.0 - 頂点グループ選択処理が遅いバグを修正。UVパイのユーティリティメニュー更新。ワイヤーフレーム自動オンが起動時に働かないバグを修正。ユーティリティペインにアニメフレームリセットを追加。ユーティリティパネルを整理。
# 1.5.0 - ディレクトリオープンボタン追加。頂点グループオーバーレイボタンを追加。先頭にアンダースコアが付いているブラシ名に変更するとエラーが発生するバグを修正。スカルプトモード変更時に自動でワイヤーフレーム化するオプションを追加。頂点複製ミラーした頂点に各情報を全てコピーするように修正。UVMap選択リスト追加。Sculpt＋マスク遷移ボタン。UVEditorパイメニュー更新。直前のモード変更ボタンを追加。ペイント：共通のサイズを使用ショートカット。
